...
public class CallActivity extends Activity implements AppRTCClient.SignalingEvents,
  ...
  // LayoutParams.FLAG_SHOW_WHEN_LOCKED are deprecated.llllllllllll
...
//    pipRenderer.setVisibility(View.GONE);
    ...
    // If capturing format is not specified for screencapture, use screen resolution. 화면 해상도
    if (screencaptureEnabled && videoWidth == 1 && videoHeight == 1) {
    ...
    matrix.setScale(-1,1);
    matrix.postTranslate(videoWidth, 0);

    Log.d(TAG, "VIDEO_FILE: '" + intent.getStringExtra(EXTRA_VIDEO_FILE_AS_CAMERA) + "'");

    // Create connection client. Use DirectRTCClient if room name is an IP otherwise use the
    // standard WebSocketRTCClient.
    //WebRTC TCP인지 확인하고 연결
